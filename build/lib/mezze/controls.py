...
def get_zero_control(num_steps):
    return np.zeros(num_steps)

def get_pi_pulse_control(num_steps, gate_duration):
    return np.ones(num_steps) * np.pi / gate_duration

def get_piover2_pulse_control(num_steps, gate_duration):
    return np.ones(num_steps) * np.pi / (2.0 * gate_duration)

def get_theta_pulse_control(num_steps, gate_duration, theta):
    return np.ones(num_steps) * theta / gate_duration
