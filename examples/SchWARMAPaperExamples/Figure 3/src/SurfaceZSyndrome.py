...
surf_zsyndrome = qecc.SurfaceSyndrome()
# Build controls
surf_zsyndrome.generate_zsyndrome_controls()
sys.stdout.flush()
...
for n in range(num_qubits):
    ctrls = circuit.generate_circuit_controls(surf_zsyndrome.ctrl_list[n])
    for cc in ctrls:
        sq_ctrls.append(cc)
tq_ctrls = []
for n in range(5, len(surf_zsyndrome.ctrl_list)):
    ctrls = circuit.generate_circuit_controls(surf_zsyndrome.ctrl_list[n])
...
qx.user_gates = {"CZZ": zz_gate}
qx.add_gate("SNOT", targets=[0])
qx.add_gate("CZZ", arg_value=-np.pi / 2.0, targets=[2, 0])
qx.add_gate("CZZ", arg_value=-np.pi / 2.0, targets=[1, 0])
qx.add_gate("CZZ", arg_value=-np.pi / 2.0, targets=[4, 0])
qx.add_gate("CZZ", arg_value=-np.pi / 2.0, targets=[3, 0])
qx.add_gate("SNOT", targets=[0])
perfect_zcircuit = qt.gate_sequence_product(qx.propagators())
zcircuit_infidelity = mezze.metrics.process_infidelity(report.channel, mezze.channel.QuantumChannel(perfect_zcircuit.full(), 'unitary'))
...
for k in range(num_realz):
    ...
    add_qth(qx_err, 0, xerr=schwarma_xangles[:, 0], yerr=schwarma_yangles[:, 0], zerr=schwarma_zangles[:, 0])
    add_qtzz(qx_err, 2, 0, xerr=schwarma_xangles[:, 1:1+NN_spec // 4], yerr=schwarma_yangles[:, 1:1+NN_spec // 4], zerr=schwarma_zangles[:, 1:1+NN_spec // 4])
    add_qtzz(qx_err, 1, 0, xerr=schwarma_xangles[:, 1+NN_spec // 4:1+2 * NN_spec // 4], yerr=schwarma_yangles[:, 1+NN_spec // 4:1+2 * NN_spec // 4], zerr=schwarma_zangles[:, 1+NN_spec // 4:1+2 * NN_spec // 4])
    add_qtzz(qx_err, 4, 0, xerr=schwarma_xangles[:, 1+2 * NN_spec // 4:1+3 * NN_spec // 4], yerr=schwarma_yangles[:, 1+2 * NN_spec // 4:1+3 * NN_spec // 4], zerr=schwarma_zangles[:, 1+2 * NN_spec // 4:1+3 * NN_spec // 4])
    add_qtzz(qx_err, 3, 0, xerr=schwarma_xangles[:, 1+3 * NN_spec // 4:1+4*NN_spec//4], yerr=schwarma_yangles[:, 1+3 * NN_spec // 4:1+4*NN_spec//4], zerr=schwarma_zangles[:, 1+3 * NN_spec // 4:1+4*NN_spec//4])
    add_qth(qx_err, 0, xerr=schwarma_xangles[:, NN_spec -1], yerr=schwarma_yangles[:, NN_spec -1], zerr=schwarma_zangles[:, NN_spec -1])
...
schwarma_zcircuit_infidelity = np.mean(schwarma_zcircuit_infidelities)
print("Mezze Z Syndrome Circuit Infidelity: " + str(zcircuit_infidelity))
print("Schwarma Z Syndrome Circuit Infidelity: " + str(schwarma_zcircuit_infidelity))

output = {"SPower": s_pow, "CorrelationTime":corr_time, "Config": config, "FullSimInfidelity":zcircuit_infidelity, "SchwARMAInfidelity":schwarma_zcircuit_infidelity, "bx":bx, "by":by, "bz":bz, "Noise Type": "XYZ Equal"}
pickle.dump(output, open(directory + 'surface_zsyndrome_xyznoise-%d-%d.p' % (int(sys.argv[1]), int(sys.argv[2])), 'wb'))
